...
Route::get('/', function () {
    return view('html.logins')->name('login');
});

Route::get('logout', [ControllerUser::class, 'logout'])->name('logout');

Route::prefix('login')->name('login.')->group(function () {
    Route::get('/', [ControllerUser::class, 'index'])->name('index')->middleware('auth');;
    Route::post('/', [ControllerUser::class, 'auth'])->name('auth')->middleware('auth');;
});
